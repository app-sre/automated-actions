#
# httpx2 adaption of https://github.com/pythongssapi/httpx-gssapi
# https://github.com/pythongssapi/httpx-gssapi/issues/57
...
from httpx2 import Auth, Request, RequestError, Response

log = logging.getLogger(__name__)
FlowGen = Generator[Request, Response]
...
class MutualAuthenticationError(RequestError):
    ...
    def __init__(self, *, request: Request | None = None, response: Response) -> None:
...
def _handle_gsserror(*, gss_stage: str, result: Any = ...) -> Callable:
    """
    Decorator to handle GSSErrors and properly log them against the decorated function's name.
    ...
    def _decor(func: Callable) -> Callable:
        @wraps(func)
...
def _gss_to_spnego_error(message: str, *args: Any, **kwargs: Any) -> NoReturn:
    ...
    try:
        ...
        raise RuntimeError("No request in arguments!") from err
...
class HTTPSPNEGOAuth(Auth):
    ...
    def auth_flow(self, request: Request) -> FlowGen:
        ctx = self.set_auth_header(request) if self.opportunistic_auth else None

        response = yield request
        yield from self.handle_response(response, ctx)

    def handle_response(
        self, response: Response, ctx: SecurityContext | None = None
    ) -> FlowGen:
        num_401s = 0
        while (
            response.status_code == _HTTP_UNAUTHORIZED and num_401s < _MAX_401_RETRIES
        ):
            num_401s += 1
        ...
        if response.status_code == _HTTP_UNAUTHORIZED or ctx is None:
            log.debug("Failed to authenticate, returning 401 response")
            return

        self.handle_mutual_auth(response, ctx)

    def handle_mutual_auth(self, response: Response, ctx: SecurityContext) -> None:
        """
        ...
        is_http_error = response.status_code >= _HTTP_ERROR_THRESHOLD
    ...
    def set_auth_header(
        ...
        """Create a new security context and insert the GSSAPI auth token into the request header.
    ...
    def _make_context(self, request: Request) -> SecurityContext:
        ...
        if type(name) is not gssapi.Name:  # None/str
